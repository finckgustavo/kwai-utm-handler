...
(function () {
  ...
  const SUPABASE_ANON_KEY =
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImVsYm1xZW13dXZkeGNobW1obXphIiwicm9sZSI6ImFub24iLCJpYXQiOjE3MzU4NzM1NjYsImV4cCI6MjA1MTQ0OTU2Nn0.K-0jDr78dQFiz1woW7BkxZKhG5J5vSubHCu3CwEONSs";
  ...
  async function getUserId() {
    try {
      const response = await fetch(
        `${SUPABASE_URL}/rest/v1/pixel_settings?pixel_id=eq.${PIXEL_ID}&select=user_id`,
        {
          headers: {
  ...
  class DatabaseManager {
    ...
    async saveTrackingEvent(params, userId) {
      ...
      try {
        const response = await fetch(
          `${this.supabaseUrl}/rest/v1/tracking_events`,
          {
            method: "POST",
            headers: {
