...
(function () {
    ...
    // Em vez de usar document.currentScript, pegamos via ID
    const currentScript = document.getElementById('utm-handler-script');
    const PIXEL_ID = currentScript?.getAttribute('data-token') || null;
    const PRODUCT_ID = currentScript?.getAttribute('data-product') || null;

    // Se quiser diagnosticar rapidamente, podemos logar:
    console.log('[Tracking] Script ID:', currentScript);
    console.log('[Tracking] PIXEL_ID:', PIXEL_ID);
    console.log('[Tracking] PRODUCT_ID:', PRODUCT_ID);

    // Get user_id from pixel_settings
    ...
    class TrackingParameters {
      ...
      getAllParams() {
        const params = {
          ...
          pixel_id: PIXEL_ID,
          product_id: PRODUCT_ID, // <-- Passando o product_id para o Supabase
